...
def put_zero_after_negative(list):
    for i in range(len(list)+1):
        if list[i] < 0:
            if i == len(list) - 1:
                list.append(0)   # вывод нуля после последнего отрицательного элемента работает через раз
            else:
                list.insert(i+1, 0) # программа ломается, если все числа положительные
...
print(new_list)
print(f'исходный массив: {new_list}\nмассив с нулевым значением после каждого отрицательного элемента: {put_zero_after_negative(new_list)}')
