...
new_positions = list(map(lambda x:x+shi,positions))  # выдает список с новыми номерами букв
translated = [alphabet_.get(number) for number in new_positions] # выдает зашифрованное слово в виде списка
