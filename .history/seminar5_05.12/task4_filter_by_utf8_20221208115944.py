...
def filter_by_sum_points(lst: list) -> List:
    '''
    отфильтрует список следующим образом:
    если сумма очков слова имеет в делителях номер, с которым она в паре в кортеже, то кортеж остается,
    его номер заменяется на сумму очков.

    Args: list
    Returns: list, sum, message
    ...
    for i in lst:
        ...
        for char in t2:
            summ += ord(char)
    summ = lambda
    is_ok = lambda t1, t2: False if summ%t1 else True
...
print(filter_by_sum_points(num_lang_lst))
